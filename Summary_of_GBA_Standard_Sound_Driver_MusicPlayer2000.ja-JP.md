...
ゲームボーイアドバンスのSDKには、ゲームで音楽や効果音を鳴らすためのサウンドドライバライブラリが添付されています。
このドライバは公式には AGB MusicPlayer2000 あるいは m4a と称されており、しばしば非公式に Sappy という名前（ポケモンサファイアに由来）でも呼ばれます。（以降は MP2k と称します）
...
|0xB9         |MEMACC     |u8 mem_set, u8 adr, u8 dat      |不可     |メモリアクセス（条件付きジャンプに使用。詳細は後述）                          |
